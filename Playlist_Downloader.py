...
def download_videos(videos):
    ...
    for video in videos:
        ...
        try:
            ...
            result_text.insert(tk.END, f"Download of {yt.title}...\n")
            result_text.config(state=tk.DISABLED)
            stream.download("videos")
            result_text.config(state=tk.NORMAL)
            result_text.insert(tk.END, f"{yt.title} Success.\n\n")
            result_text.config(state=tk.DISABLED)
        except Exception as e:
            result_text.config(state=tk.NORMAL)
            result_text.insert(tk.END, f"Error downloading {yt.title}: {e}\n\n")
            result_text.config(state=tk.DISABLED)
...
def fetch_videos():
    ...
    try:
        ...
        result_text.insert(tk.END, "Video Playlist Name :\n")
        for video in videos:
        ...
        result_text.config(state=tk.NORMAL)
        result_text.delete('1.0', tk.END)
        result_text.insert(tk.END, f"An error has occurred : {e}\n")
        result_text.config(state=tk.DISABLED)
...
api_key_label = tk.Label(window, text="Enter API YouTube:")
api_key_label.pack()
api_key_entry = tk.Entry(window, width=40)
api_key_entry.pack()

playlist_link_label = tk.Label(window, text="Link of Youtube playlist:")
playlist_link_label.pack()
playlist_link_entry = tk.Entry(window, width=40)
playlist_link_entry.pack()

fetch_button = tk.Button(window, text="Recover videos", command=fetch_videos)
fetch_button.pack()
